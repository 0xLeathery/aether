...
impl NetworkService {
    ...
    /// Start the network service with a pre-shared key for swarm isolation
    ///
    /// Loads keypair from keychain, builds swarm with PSK, starts listening, and spawns event loop
    pub fn start_with_psk(&mut self, app: AppHandle, psk: libp2p::pnet::PreSharedKey) -> Result<(), NetworkError> {
        if self.is_running() {
            return Err(NetworkError::AlreadyRunning);
        }
